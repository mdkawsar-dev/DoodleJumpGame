...
public class MultiplayerGame extends JPanel implements ActionListener, KeyListener {
    ...
    private Socket socket;  //network
    private PrintWriter out;
    private BufferedReader in;
    private boolean isHost;

    public MultiplayerGame(boolean isHost) {
        this.isHost = isHost;
...
private void setupNetwork() {
   try {
        if (isHost) {
         ...
         JOptionPane.showMessageDialog(this, "Waiting for Player 2...\nTell them to connect to your IP");
         socket = serverSocket.accept();
          JOptionPane.showMessageDialog(this, "Player 2 connected! Starting game...");
            }
        else {
          String ip = JOptionPane.showInputDialog("Enter host IP (or localhost):");
          if (ip == null || ip.trim().isEmpty()) ip = "localhost";
           socket = new Socket(ip, 12345);
           JOptionPane.showMessageDialog(this, "Connected! Starting game...");
            }
 ...
 private void networkListener() {
  try {
   ...
   opponent = new Entity(0, 0, 30, 40, new Color(200, 100, 100));
      }
   opponent.setX(Double.parseDouble(parts[0]));
   opponent.setY(Double.parseDouble(parts[1]));
   opponent.setVelocityX(Double.parseDouble(parts[2]));
   opponent.setVelocityY(Double.parseDouble(parts[3]));
    ...
    private void sendData() {
        if (out != null && player != null) {
            out.println(player.getX() + "," + player.getY() + "," +
                    player.getVelocityX() + "," + player.getVelocityY() + "," + score);
        }
    }
    ...
    private void startGame() {
        ...
        if (isHost) {
            player = new Entity(startX + 10, startY - 40, 30, 40, new Color(100, 100, 200));
        } else {
            player = new Entity(startX + 60, startY - 40, 30, 40, new Color(100, 200, 100));
        }
    ...
    public void paintComponent(Graphics g) {
        ...
        if (opponent != null) {
            g.setColor(Color.BLACK);
            g.fillOval((int) opponent.getX() + 7, (int) opponent.getY() + 8, 6, 6);
            g.fillOval((int) opponent.getX() + 17, (int) opponent.getY() + 8, 6, 6);
            g.drawArc((int) opponent.getX() + 8, (int) opponent.getY() + 20, 14, 8, 0, -180);
        }

        g.setColor(Color.BLACK);
        g.setFont(new Font("Arial", Font.BOLD, 16));
        g.drawString("You: " + score, 10, 25);
        g.drawString("Opponent: " + opponentScore, 10, 45);
        g.drawString("Level: " + difficultyLevel, 10, 65);


        g.setColor(Color.WHITE);
        g.setFont(new Font("Arial", Font.BOLD, 10));
        g.drawString("YOU", (int) player.getX(), (int) player.getY() - 5);
        if (opponent != null) {
            g.drawString("P2", (int) opponent.getX(), (int) opponent.getY() - 5);
        }
        ...
        if (!gameStarted || gameOver) {
            ...
            if (gameOver) {
                ...
                String result = score > opponentScore ? "You Win!" :
                        score < opponentScore ? "You Lose!" : "Tie!";
                textWidth = g.getFontMetrics().stringWidth(result);
                g.drawString(result, WIDTH / 2 - textWidth / 2, HEIGHT / 2);
            } else {
                String titleText = "Multiplayer Doodle Jump";
    ...
    private void updateGame() {
        ...
        double moveSpeed = 4 + difficultyLevel * 0.5;
        if (leftPressed) {
...
if (opponent != null) {
  opponent.setY(opponent.getY() + offset);
    }
